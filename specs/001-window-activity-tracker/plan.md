...
| VI. Security | ✅ PASS | Tauri capabilities system enforces least privilege. All IPC inputs validated in Rust handlers. Connection URI stored using platform keychain (not plain text). No unauthenticated local endpoints. Threat model recorded in [research.md](research.md). **Scope clarification**: Constitution VI's "trace data MUST be encrypted at rest" is interpreted to exclude the local SQLite cache — screenshots are stored as plain files under OS folder permissions per explicit spec intent (FR-018), and the SQLite DB mirrors data already in the user-supplied external DB. The sensitive credential (connection URI) is fully protected via OS keychain (`keyring` crate). |
...
| IV. Performance | ✅ PASS | SQLite WAL mode enabled. Window activity batch-flushed every **30 seconds** (SC-007). Screenshot pipeline in `spawn_blocking`. Sync runs every 30 seconds on background task. |
